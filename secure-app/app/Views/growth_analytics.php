<?= $this->include('templates/header', ['title' => 'Growth Analytics']) ?>
...
<div class="glass-panel p-8 md:p-12 rounded-2xl shadow-2xl max-w-7xl w-full relative z-10 mx-4 my-8">
    ...
    <!-- Filter Form -->
    <form action="/growth" method="GET" class="mb-6 flex flex-col md:flex-row items-end gap-4 bg-slate-900/40 p-4 rounded-xl border border-slate-800 justify-between">
        <div class="flex items-center gap-4 flex-1">
            <div>
                <label class="block text-slate-400 text-xs font-bold mb-1">Filter by Player:</label>
                <select name="user_id" class="w-64 bg-slate-950/50 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-emerald-500 transition-colors appearance-none">
                    <option value="">-- All Players --</option>
                    <?php foreach ($users_dropdown as $u): ?>
                        <option value="<?= $u['id'] ?>" <?= $selected_user == $u['id'] ? 'selected' : '' ?>>
                            <?= !empty($u['player_name']) ? esc($u['player_name']) : esc($u['username']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
                ...
                <select name="days_limit" class="bg-slate-950/50 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-emerald-500 transition-colors appearance-none">
                        ...
                        <?php foreach ($growth_history as $log): ?>
                            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right font-mono font-bold text-slate-300">
                                    <?= number_format($log['total_power'], 2) ?>
                                </td>

                                <td class="px-6 py-4 text-right font-mono font-bold text-sm <?= $log['total_growth'] > 0 ? 'text-emerald-400' : ($log['total_growth'] < 0 ? 'text-rose-400' : 'text-slate-500') ?>">
                                    <?= $log['total_growth'] > 0 ? '+' : '' ?><?= number_format($log['total_growth'], 2) ?>
                                </td>
    ...
    <div class="mt-8 text-center flex justify-center gap-4">
        <a href="/dashboard" class="inline-flex items-center text-sm text-slate-400 hover:text-emerald-400 transition-colors">
            ← Back to Dashboard
        </a>
    </div>
</div>
